...
void main () {
    ...
    float quantNum = 30.;
    vec2 warpN = mix(stN, coordWarp(stN, 50.+time/100., 20.).xy, 0.9);
    vec2 quantN = quant2(warpN, quantNum);
    
    float t = time * 0.3;
    vec2 ball = vec2(sin(t), cos(t*2.3))*0.3+0.5;
    bool inBall = distance(stN, ball) < 0.04 && mod(time, 1.) < 0.01;
    
    float dev = 0.0015;
    vec2 fdbkN = mod(quantN.x*quantNum, 2.) < 0.01 ? stN+vec2(0., dev) : stN-vec2(0., dev);
    vec2 fdbkN2 = mod(quantN.y*quantNum, 2.) < 0.01 ? fdbkN+vec2(dev, 0.) : fdbkN-vec2(dev, 0.);
    vec4 bb = texture2D(backbuffer, mix(warpN, fdbkN2, 0.999));
    
    vec3 c = inBall ? white*sinN(time*10.7) : bb.rgb;
