...
void ex3() {
    ...
    vec4 bbAvg = avgColorBB(mix(stN, warpN, 0.005), 0.00, 0.002);
    ...
    vec3 hashN = hash(vec3(stN, .5)) / max(1000., pow(distance(stN, circ), .1)*100.);
    
    condition = distance(mix(stN + hashN.xy*100., warpN, 0.), circ) > rad;
    
    vec3 c;
    float feedback; 
    if(condition){
        feedback = lastFeedback * 0.98;
    } 
    else{
        feedback = 1.;
    }
    ...
    float noiseval = snoise(vec3(stN*2. + hashN.xy*100., time/5.));
    c = noiseval  > 0.3 ? black : mix(black, white, 1.);
    
    vec3 col = feedback == 1. ? c : bbAvg.rgb;
    float trailCut = 0.05;
