...
void main () {
    ...
    float ns = snoise(vec2(t3, 0.5));
    dev = vec2(ns, cos(time*1.3))*0.3;
    ...
    float feedback;
    vec2 quantN = quant(stN, 3.+ quant2(mod(time*0.07, 1.), 2.)*600.);
    vec2 dev1 = vec2(snoise(vec3(quantN, t2)), snoise(vec3(quantN, 80.*0.2)))*0.7;
    vec2 devN = mix((hash(vec3(stN, 5.)).xy-0.5), dev1, sinN(time)*0.1+0.8)*0.02;
    vec4 bb = texture2D(backbuffer, stN+devN);
    
    float lastFeedback = bb.a;
    float decay = 0.01;
    bool condition =  col == white ;
    vec3 brush  = mod(ns, .5) < 0.25 ? black : white;
    brush = vec3(sinN(distance(cent, coordWarp(stN, time).xy)*PI*10.+time));
    col = distance(cent, stN)< 0.1 ? brush : bb.rgb;
    // col = vec3(quant2(mod(time*0.1, 1.), 2.));
    // if(condition){
    //     feedback = 1.;
    // } else {
    ...
    // col = distance(cent, stN) < 0.1 && mod(ns, .5) < 0.25 ? black : white;
    gl_FragColor = vec4(col, bb.r);
}