...
vec3 thresh(vec3 col){
    return vec3(lum(col).x > 0.5);
}
...
void ex3() {
    ...
    vec2 modN;
    vec2 twist = ballTwist(stN, time/5.).xy;
    vec2 twistLite = mix(stN, twist, 0.2);
    float inc = 0.1 + sinN(time/3.);
    modN = mix(stN, vec2(0.5), 1.*inc);
    vec3 vid = texture2D(channel5, stN).rgb;
    vec3 vidSnap = texture2D(channel7, modN).rgb;
    
    
    vec3 vid2 = texture2D(channel5, mix(twistLite, vec2(0.5), 2.*inc)).rgb;
    vec3 vidSnap2 = texture2D(channel7, mix(twistLite, vec2(0.5), 3.*inc)).rgb;
    ...
    if(dist < .3 + sinN(time)*0.3){
        feedback = bb.a > .9 ? bb.a - 0.35 : bb.a - 0.04;
    } 
    else{
        feedback = 1.;
    } 
    ...
    vid = thresh(vid);
    vidSnap = thresh(vidSnap);
    vid2 = thresh(vid2)*red;
    vidSnap2 = thresh(vidSnap2)*red;
    ...
    gl_FragColor = vec4(scrollLayer, feedback);//vec4(c, feedback);
}
