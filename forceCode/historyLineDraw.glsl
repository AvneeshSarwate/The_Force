...
vec2 brushMovement(float t){ //argument time
    return vec2(sinN((cosN(t/5.52)* 20. + sinN(t/6.32)*305.)), 
                     cosN((sinN(t/1.52)* 20. + cosN(t/1.32)*250.)));
}

vec2 brushMove(float time){
    return vec2(sinN(time*2.), cosN(time));
}

bool shouldDraw(vec2 stN, vec2 brushPoint){
    return distance(stN, brushPoint) < 0.05;
}
