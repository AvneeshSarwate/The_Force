...
void ex3() {
    ...
    vec3 vid = texture2D(channel5, mix(stN, modN, sliderVals[1])).rgb;
    vec3 vidSnap = texture2D(channel7, mix(stN, modN, sliderVals[2])).rgb;
    // modN = mix(stN, modN, sigmoid(sin(time+stN.x*20.*PI)*100.));
    vec3 vidnoise = texture2D(channel5, stN).rgb;
