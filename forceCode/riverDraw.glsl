...
void main () {
    ...
    float quantNum = 10.;
    vec2 warpN = mix(stN, coordWarp(stN, 50., 20.).xy, sinN(time/5.));
    vec2 quantN = quant2(warpN, quantNum);
    
    vec2 ball = vec2(sin(time), cos(time))*0.3+0.5;
    vec2 circ = vec2(sinN(time), cosN(time))*0.5 + .25;
    vec2 mouseN = mouse.xy/2./resolution.xy;
    circ = vec2(mouseN.x, 1.-mouseN.y);
    float rad = 0.1;
    bool inBall = distance(stN, circ) < 0.1;
    
    float xDelt = 0.001 + rand(stN.x*10.)*0.0003;
    vec2 noiseN = vec2(snoise(vec3(stN.xx, 10.)), snoise(vec3(stN.xx-xDelt, 10.)))/(sqrt(.75));
    
    float yDev = noiseN.x;
    float yDelt = noiseN.x - noiseN.y;
    
    bool inStrip = abs(stN.y- (0.3+yDev*0.4)) < 0.2;
    
    vec2 fdbkN = inStrip ? stN - vec2(xDelt, yDelt) : stN;
    vec4 bb = texture2D(backbuffer, fdbkN);
    
    vec3 c = inBall ? vec3(snoise(vec3(stN*10., time))) : bb.rgb;
    c = inStrip || inBall ? c : black;
    // c = vec3(snoise(vec3(fdbkN*10., 10.)));
