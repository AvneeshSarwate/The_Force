...
vec2 fmWave(float t){ //argument time
    return vec2(sinN((cosN(t/5.52) + sinN(t/6.32)*305.)/50.), 
                     cosN((sinN(t/1.52) + cosN(t/1.32)*250.)/500.))/2.;
}
...
void main () {
    ...
    float decay2 = 0.5;
    float feedback;
    vec4 bb = texture2D(backbuffer, mix(hashN, rotN, 0.01*pow(rotN.y, .02)));
    ...
    float randHue = 5.;
    float dev = 0.09;
    vec3 c1 = hsv2rgb(vec3(randHue+dev, .5, .8));
    vec3 c2 = hsv2rgb(vec3(randHue-dev, .5, .8));
    vec3 c3 = hsv2rgb(vec3(randHue+0.5, .5, .8));
    float modTime = mod(time/5. + warpCoord.x*PI + col.x, 3.);
    float colorInd = floor(modTime);
    vec3 color;
    if(colorInd == 0.) color = c1;
    if(colorInd == 1.) color = c2;
    if(colorInd == 2.) color = c3;
    
    // float numQuant = 4.;
    // vec2 quantN = quant(mix(stN, warpCoord, 0.), numQuant);
    // float quantInd = floor(quantN.y*numQuant)*numQuant + floor(quantN.x*numQuant);
    // bool cellIsSelected = mod(floor(time*1.), numQuant*numQuant) ==quantInd;
    // cellIsSelected = distance(warpCoord, quantN) < 1./numQuant/50. + sinN(time)*0.01;
    
    // color = (cellIsSelected && condition ? mix(white, color, mod(time, 1.)) : white); 
    // color = cellIsSelected ? color : white;
    
    col = mix(bb.rgb, col*color*(distance(stN, warpCoord)*8.), 0.1);
