...
float vecSelect(vec4 v, float i){
    if(0. <= i && i < 1.) return mix(v.r, v.g, i);
    if(1. <= i && i < 2.) return mix(v.g, v.b, mod(i, 1.));
    if(2. <= i && i < 3.) return mix(v.b, v.a, mod(i, 1.));
    if(3. <= i && i < 4.) return mix(v.a, v.r, mod(i, 1.));
}

float draw (vec2 stN, float t, float i) {
    ...
    float lastFeedback = vecSelect(bb, mod(i+time/10., 4.));

    // vec2 multBall = multiBallCondition(stN, t2/2.);
    float t3 = t/3.;
    float zoomScale = 0.5;
    float numLev = 100.*sinN(t/12.) + 10.; //vs just 10.
    float posLineDif = .01;
    float lineGranularity = 10.;
    bool condition = mod(stN.y, 1./lineGranularity)  < 0.5/lineGranularity;
...
void main () {
    vec2 stN = uvN();
    vec2 warpN = coordWarp(stN, time/4.).xy;
    vec2 warpN2 = coordWarp(stN, -time/40.).xy;
    float f0 = draw(stN+time/30., time+0.0, 0.);
    float f1 = draw(mix(stN, warpN, 0.1), time+0.1, 1.);
    float f2 = draw(stN+0.2, time+0.2, 2.);
    float f3 = draw(warpN2, time+0.3, 3.);

    
    gl_FragColor = vec4(f0, f1, f2, f3);
}