...
class Point{
    ...
    draw(time){
        var param = 1; //this.sinkId == 0 ? 0 : 1;
        var swellSize = param == 0 ? 100 : 50;
        stroke(250);
        strokeWeight(2);
        fill(0);
        var ballSize = this.size + rampAD(1-sliderVals[param], 0.05) * swellSize;
...
//input time must be between 0-1, peak-time must be beterrn 0-1
function rampAD(inputTime, peakTime){
